...
```bash
NEXT_PUBLIC_API_BASE_URL=https://transit-link-fix.preview.emergentagent.com
```
...
```bash
curl "https://transit-link-fix.preview.emergentagent.com/api/airports?query=pu&limit=10"
...
**Expected Behavior:**
1. User navigates to: `https://transit-link-fix.preview.emergentagent.com/`
2. Types "pu" in the From field
3. DevTools Network tab shows request to:
   ```
   https://transit-link-fix.preview.emergentagent.com/api/airports?query=pu&limit=10
   ```
...
### Mumbai Test
```bash
curl "https://transit-link-fix.preview.emergentagent.com/api/airports?query=mum&limit=10"
...
1. **`.env.local`**
   ```diff
   - NEXT_PUBLIC_API_URL=https://transit-link-fix.preview.emergentagent.com
   + NEXT_PUBLIC_API_BASE_URL=https://transit-link-fix.preview.emergentagent.com
   ```
...
curl "https://transit-link-fix.preview.emergentagent.com/api/airports?query=pu"
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://transit-link-fix.preview.emergentagent.com"],
...
**Homepage:**
```
https://transit-link-fix.preview.emergentagent.com/
```
...
**Flight Search API:**
```
https://transit-link-fix.preview.emergentagent.com/api/search/flights?origin=PNQ&destination=BOM&departure_date=2025-12-07&adults=1
```

**API Documentation:**
```
https://transit-link-fix.preview.emergentagent.com/api/docs
```
...
# Test backend API
curl "https://transit-link-fix.preview.emergentagent.com/api/health"
```
